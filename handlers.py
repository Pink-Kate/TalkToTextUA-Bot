...
# Імпортуємо залежності з обробкою помилок
try:
    from config import MAX_AUDIO_DURATION
except ImportError as e:
    raise ImportError(f"Не вдалося імпортувати config: {e}") from e

try:
    from storage import add_to_history, clear_chat_history, get_chat_history, get_user_settings
except ImportError as e:
    raise ImportError(f"Не вдалося імпортувати storage: {e}") from e

try:
    from transcription import download_audio_file, transcribe_audio
except ImportError as e:
    raise ImportError(f"Не вдалося імпортувати transcription: {e}") from e
...
try:
    ...
    raise ImportError(f"Не вдалося імпортувати utils: {e}") from e
