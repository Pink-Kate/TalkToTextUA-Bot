...
async def transcribe_audio(audio_path: str, user_id: int | None = None, audio_duration: int | None = None):
    ...
    # Таймаути встановлені для надійної обробки, але не занадто великі
    if audio_duration:
        if audio_duration <= 5:  # дуже короткі (до 5 сек)
            timeout = 120  # 2 хвилини - достатньо для швидкої обробки
        elif audio_duration <= 10:  # до 10 секунд
            timeout = 180  # 3 хвилини
        elif audio_duration <= 30:  # до 30 секунд
            timeout = 240  # 4 хвилини
    ...
    # Менші значення best_of та beam_size = швидше, але з temperature=0.0 якість залишається високою
        ...
        with model_lock:
            ...
            if target_lang:
                ...
                try:
                    ...
                    # Для українського бота: якщо target_lang не "uk", то перекладаємо
                    use_translate = target_lang != "uk"
                    if use_translate:
