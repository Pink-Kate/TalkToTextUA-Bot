...
async def transcribe_audio(audio_path: str, user_id: int | None = None, audio_duration: int | None = None):
    ...
    # Для дуже коротких файлів використовуємо мінімальні параметри для максимальної швидкості
    ...
    def run():
        ...
        with model_lock:
            ...
            # Для дуже коротких файлів використовуємо максимально чутливі та швидкі параметри
            if audio_duration and audio_duration <= 5:  # дуже короткі (до 5 сек)
                ...
                base_params.update({
                    "no_speech_threshold": 0.15,  # Дуже низький поріг - приймаємо навіть тихі звуки
                    "compression_ratio_threshold": 2.4,
                })
            elif audio_duration and audio_duration <= 30:  # короткі (до 30 сек)
